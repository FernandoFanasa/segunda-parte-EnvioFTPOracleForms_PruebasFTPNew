...
{
    public partial class Atenea : Form
    {
        public Atenea()
        {
            InitializeComponent();
        ...
        private void getOrigenes()
        {
            try
            {
                ...
                var buscar = from c in a.ConfiguracionOrigen3WM select c;
        ...
        private void getdatos()
        {
            try
            {
                dataGridViewAtenea.DataSource = "";
                COBDDataContext a = new COBDDataContext();
                //var buscar = from c in a.OrigenDatosFanasa select c;
                //dataGridViewOracle.DataSource = buscar;


                dataGridViewAtenea.DataSource = from c in a.OrigenDatos3WM
                                                join r in a.ConfiguracionOrigen3WM on c.iRegistroConfigOrigen equals r.uiRegistroOrigen
                                                select new
                                                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (lblID.Text != "-")
            {
                if (!checkBoxFESA.Checked && !checkBoxFANASA.Checked)
                {
                    DialogResult result;
                    result = MessageBox.Show("Debe seleccionar el tipo de FTP (FESA-FANASA)", "Error al guardar");
                }
                else if (checkBoxFESA.Checked && checkBoxFANASA.Checked)
                {
                    DialogResult result;
                    result = MessageBox.Show("Debe seleccionar solo UN FTP (FESA-FANASA)", "Error al guardar");
                }
                else
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscar = from c in a.OrigenDatos3WM where c.IdOrigenDatos == id select c;
                    ...
                    buscar.First().bFESA = Convert.ToBoolean(checkBoxFESA.CheckState);
                    buscar.First().bFANASA = Convert.ToBoolean(checkBoxFANASA.CheckState);
                        ...
                        if (txtBxDestino.Text != "" || txtBxDesFTPPaso.Text != "")
                        {
                            if (txtBxDestino.Text != "" && txtBxDesFTPPaso.Text == "")
                            {
                                if (comboBoxOrigen.SelectedIndex != -1)
                                {
                                    ...
                                    OrigenDatos3WM n = new OrigenDatos3WM();
                                    ...
                                    n.bFESA = Convert.ToBoolean(checkBoxFESA.CheckState);
                                    n.bFANASA = Convert.ToBoolean(checkBoxFANASA.CheckState);
                                    a.OrigenDatos3WM.InsertOnSubmit(n);
                                    a.SubmitChanges();
                                    limpiar();
                                    getdatos();
                                }
                            ...
                            else
                            {
                                ...
                                {
                                    var ip = txtBxIPFTP.Text;
                                    var user = txtBxUserFTP.Text;
                                    var pass = txtBxPassFTP.Text;
                                    var combo = comboBox1.SelectedIndex;
                                    var puerto = txtBxPuerto.Text;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (lblID.Text != "-")
            {
                ...
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscarcorreos = from c in a.CtlCorreos3WM where c.IdOrigenDatos3WM == id select c;
                    foreach (var item in buscarcorreos)
                    {
                        a.CtlCorreos3WM.DeleteOnSubmit(item);
                        a.SubmitChanges();
                    }
                    var buscar = from c in a.OrigenDatos3WM where c.IdOrigenDatos == id select c;
                    a.OrigenDatos3WM.DeleteOnSubmit(buscar.First());
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
        ...
        {
            this.Hide();
            CorreosAtenea correoAtenea = new CorreosAtenea();
            correoAtenea.Show();
        }
        ...
        {
            this.Hide();
            OrigenAtenea origenAtenea = new OrigenAtenea();
            origenAtenea.Show();
        }
        ...
        private void dataGridViewAtenea_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex == -1)
            {
                limpiar();
            ...
            {
                DataGridViewRow row = (DataGridViewRow)dataGridViewAtenea.Rows[e.RowIndex];
                if (row != null)
                {
                    limpiar();
        ...
        private void Atenea_Load(object sender, EventArgs e)
        {
            getdatos();
            getOrigenes();
        }
