...
{
    public partial class CarvajalFANASA : Form
    {
        public CarvajalFANASA()
        {
            InitializeComponent();
        }

        private void CarvajalFANASA_Load(object sender, EventArgs e)
        {
            getdatos();
            getOrigenes();
        }
        ...
        private void getdatos()
        {
            try
            {
                ...
                dataGridViewCavajal.DataSource = from c in a.OrigenDatosCarvajalFANASA
                                                join r in a.ConfiguracionOrigenCarvajalFANASA on c.iRegistroConfigOrigen equals r.uiRegistroOrigen
                                                select new
                                                {
        ...
        private void getOrigenes()
        {
            ...
            {
                COBDDataContext a = new COBDDataContext();
                var buscar = from c in a.ConfiguracionOrigenCarvajalFANASA select c;
                    ...
                    if (txtBxDestino.Text != "" || txtBxDestinoRespaldo.Text != "")
                    {
                             ...
                             txtBxPassFTP.Text == "" && txtBxPuerto.Text == "")
                        {
                            if (comboBoxOrigen.SelectedIndex != -1)
                            {
                                ...
                                OrigenDatosCarvajalFANASA n = new OrigenDatosCarvajalFANASA();
                                ...
                                a.OrigenDatosCarvajalFANASA.InsertOnSubmit(n);
                                a.SubmitChanges();
                                limpiar();
                                getdatos();
                            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (lblID.Text != "-")
            {
                ...
                var buscar = from c in a.OrigenDatosCarvajalFANASA where c.IdOrigenDatos == id select c;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (lblID.Text != "-")
            {
                ...
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscarcorreos = from c in a.CtlCorreosCarvajalFANASA where c.IdOrigenDatos == id select c;
                    foreach (var item in buscarcorreos)
                    {
                        a.CtlCorreosCarvajalFANASA.DeleteOnSubmit(item);
                        a.SubmitChanges();
                    }
                    var buscar = from c in a.OrigenDatosCarvajalFANASA where c.IdOrigenDatos == id select c;
                    a.OrigenDatosCarvajalFANASA.DeleteOnSubmit(buscar.First());
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
        ...
        private void configuraciónOrigenCarvalFANASAToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.Hide();
            OrigenCarvajalFANASA OrigenCarvajalFANASA = new OrigenCarvajalFANASA();
            OrigenCarvajalFANASA.Show();
        }

        private void correosCarvajalFANASAToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.Hide();
            CorreosCarvajalFANASA correosFANASA = new CorreosCarvajalFANASA();
            correosFANASA.Show();
        }
