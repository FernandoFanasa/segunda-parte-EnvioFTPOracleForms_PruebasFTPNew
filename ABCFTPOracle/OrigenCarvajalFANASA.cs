...
{
    public partial class OrigenCarvajalFANASA : Form
    {
        public OrigenCarvajalFANASA()
        {
            InitializeComponent();
        }

        private void OrigenCarvajalFANASA_Load(object sender, EventArgs e)
        {
            getdatos();
        }
        ...
        private void getdatos()
        {
            try
            {
                dataGridViewOrigenCravajal.DataSource = "";
                COBDDataContext a = new COBDDataContext();

                dataGridViewOrigenCravajal.DataSource = from c in a.ConfiguracionOrigenCarvajalFANASA
                                                select new
                                                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (lblID.Text == "-")
            {
                ...
                ConfiguracionOrigenCarvajalFANASA c = new ConfiguracionOrigenCarvajalFANASA();
                ...
                a.ConfiguracionOrigenCarvajalFANASA.InsertOnSubmit(c);
                a.SubmitChanges();
                limpiar();
                getdatos();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (lblID.Text != "-")
            {
                ...
                var buscar = from c in a.ConfiguracionOrigenCarvajalFANASA where c.uiRegistroOrigen == id select c;
                if (txtBxOrigen.Text != "")
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (lblID.Text != "-")
            {
                ...
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscarOrigen = from c in a.ConfiguracionOrigenCarvajalFANASA where c.uiRegistroOrigen == id select c;
                    a.ConfiguracionOrigenCarvajalFANASA.DeleteOnSubmit(buscarOrigen.First());
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
        ...
        private void dataGridViewOrigenCravajal_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex == -1)
            {
                limpiar();
            ...
            {
                DataGridViewRow row = dataGridViewOrigenCravajal.Rows[e.RowIndex];
                if (row != null)
                {
                    limpiar();
        ...
        private void configuraciónCarvajalFANSAToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.Hide();
