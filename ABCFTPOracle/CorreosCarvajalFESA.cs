...
{
    public partial class CorreosCarvajalFESA : Form
    {
        public CorreosCarvajalFESA()
        {
            InitializeComponent();
        }

        private void CorreosCarvajalFESA_Load(object sender, EventArgs e)
        {
            getdatos();
            getFields();
        }
        ...
        private void getdatos()
        {
            try
            {
                ...
                //             select c;
                dataGridViewCorreosCarvajal.DataSource = from c in a.CtlCorreosCarvajalFESA
                                                         join r in a.OrigenDatosCarvajalFESA on c.IdOrigenDatos equals r.IdOrigenDatos
                                                         select new
                                                         {
                                                             IdCorreo = c.IdCorreos,
                                                             IdOrigenDatos = c.IdOrigenDatos,
                                                             Field1 = c.OrigenDatosCarvajalFESA.Field1,
                                                             Field2 = c.OrigenDatosCarvajalFESA.Field2,
                                                             Field3 = c.OrigenDatosCarvajalFESA.Field3,
                                                             Correo = c.Correo
                                                         };
            }
            catch (Exception ex)
        ...
        private void getFields()
        {
            ...
            {
                COBDDataContext a = new COBDDataContext();
                var buscar = from c in a.OrigenDatosCarvajalFESA select c;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (lblID.Text == "-")
            {
                ...
                if (cmbBoxLab.SelectedIndex != -1)
                {
                    ...
                    CtlCorreosCarvajalFESA n = new CtlCorreosCarvajalFESA();
                    n.IdOrigenDatos = Convert.ToInt32(desgloce[0].ToString());
                    n.Correo = txtCorreo.Text;
                    a.CtlCorreosCarvajalFESA.InsertOnSubmit(n);
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (lblID.Text != "-")
            {
                ...
                var buscar = from c in a.CtlCorreosCarvajalFESA where c.IdCorreos == id select c;

                var bucarDato = (from c in a.CtlCorreosCarvajalFESA where c.IdCorreos == id select c).FirstOrDefault();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (lblID.Text != "-")
            {
                ...
                if (result == System.Windows.Forms.DialogResult.Yes)
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscar = from c in a.CtlCorreosCarvajalFESA where c.IdCorreos == id select c;
                    a.CtlCorreosCarvajalFESA.DeleteOnSubmit(buscar.First());
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
        ...
        private void envioCarvajalFESAToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.Hide();
