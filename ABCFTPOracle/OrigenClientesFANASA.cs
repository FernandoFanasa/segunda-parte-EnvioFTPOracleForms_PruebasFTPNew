...
{
    public partial class OrigenClientesFANASA : Form
    {
        public OrigenClientesFANASA()
        {
            InitializeComponent();
        }
        private void OrigenClientesFANASA_Load(object sender, EventArgs e)
        {
            getdatos();
        }
        ...
        private void getdatos()
        {
            try
            {
                dataGridViewOrigenClientes.DataSource = "";
                COBDDataContext a = new COBDDataContext();

                dataGridViewOrigenClientes.DataSource = from c in a.ConfiguracionOrigenClientesFANASA
                                                        select new
                                                        {
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            if (lblID.Text == "-")
            {
                ...
                ConfiguracionOrigenClientesFANASA c = new ConfiguracionOrigenClientesFANASA();
                ...
                a.ConfiguracionOrigenClientesFANASA.InsertOnSubmit(c);
                a.SubmitChanges();
                limpiar();
                getdatos();
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (lblID.Text != "-")
            {
                ...
                var buscar = from c in a.ConfiguracionOrigenClientesFANASA where c.uiRegistroOrigen == id select c;
                if (txtBxOrigen.Text != "")
                {
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            if (lblID.Text != "-")
            {
                ...
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscarOrigen = from c in a.ConfiguracionOrigenClientesFANASA where c.uiRegistroOrigen == id select c;
                    a.ConfiguracionOrigenClientesFANASA.DeleteOnSubmit(buscarOrigen.First());
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
            ...
            {
                DialogResult result;
                result = MessageBox.Show("No esta selecionado un registro, favor de seleccionarlo", "Error al guardar");
            }
        }

        private void dataGridViewOrigenClientes_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex == -1)
            {
                limpiar();
            ...
            {
                DataGridViewRow row = dataGridViewOrigenClientes.Rows[e.RowIndex];
                if (row != null)
                {
                    limpiar();
        ...
        private void configuraciónClientesFANSAToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.Hide();
            ClientesFANASA clienteFANASA = new ClientesFANASA();
            clienteFANASA.Show();
        }
