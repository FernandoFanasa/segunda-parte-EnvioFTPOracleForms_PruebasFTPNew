...
{
    public partial class CorreosEtiquetas : Form
    {
        public CorreosEtiquetas()
        {
            InitializeComponent();
        }

        private void CorreosEtiquetas_Load(object sender, EventArgs e)
        {
            getdatos();
            getFields();
        }
        ...
        private void getdatos()
        {
            try
            {
                ...
                //             select c;
                dataGridView1.DataSource = from c in a.CtlCorreosEtiquetas
                                           join r in a.OrigenDatosEtiquetas on c.IdOrigenDatosEtiquetas equals r.IdOrigenDatos
                                           select new
                                           {
                                               IdCorreo = c.IdCorreos,
                                               IdOrigenDatosNew = c.OrigenDatosEtiquetas,
                                               Field1 = c.OrigenDatosEtiquetas.Field1,
                                               Field2 = c.OrigenDatosEtiquetas.Field2,
                                               Field3 = c.OrigenDatosEtiquetas.Field3,
                                               Correo = c.Correo
                                           };
        ...
        private void getFields()
        {
            ...
            {
                COBDDataContext a = new COBDDataContext();
                var buscar = from c in a.OrigenDatosEtiquetas select c;
                foreach (var item in buscar)
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (lblID.Text == "-")
            {
                ...
                if (cmbBoxLab.SelectedIndex != -1)
                {
                    ...
                    CtlCorreosEtiquetas n = new CtlCorreosEtiquetas();
                    n.IdOrigenDatosEtiquetas = Convert.ToInt32(desgloce[0].ToString());
                    n.Correo = txtCorreo.Text;
                    a.CtlCorreosEtiquetas.InsertOnSubmit(n);
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (lblID.Text != "-")
            {
                ...
                var buscar = from c in a.CtlCorreosEtiquetas where c.IdCorreos == id select c;

                var bucarDato = (from c in a.CtlCorreosEtiquetas where c.IdCorreos == id select c).FirstOrDefault();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (lblID.Text != "-")
            {
                ...
                if (result == System.Windows.Forms.DialogResult.Yes)
                {
                    int id = Convert.ToInt32(lblID.Text);
                    var buscar = from c in a.CtlCorreosEtiquetas where c.IdCorreos == id select c;
                    a.CtlCorreosEtiquetas.DeleteOnSubmit(buscar.First());
                    a.SubmitChanges();
                    limpiar();
                    getdatos();
                }
